...
StringIntMap makeWordCounts(const StringVec &words)
{
  ...
  for (auto word : words)
  {
    if (auto search = wordcount_map.find(word); search != wordcount_map.end())
    {
      search->second = search->second + 1;
    }
    else
    {
      wordcount_map.insert({word, 1});
    }
  }
...
int lookupWithFallback(const StringIntMap &wordcount_map, const std::string &key, int fallbackVal)
{
  ...
  // =================================================
  if (auto search = wordcount_map.find(key); search != wordcount_map.end())
  {
